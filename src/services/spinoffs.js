...
const {getAllSpinoffsFromDB} = require("../repositories/spinoffs");
const ObjectId = require('mongodb').ObjectId;


const getAllSpinoffs = async (req, res) => {
    return getAllSpinoffsFromDB().then(
        (lists) => {
...
const getSingleSpinoffs = async (req, res, next) => {
    if (!ObjectId.isValid(req.params.id)) {
        res.status(400).json('Must use a valid spinoffs id');
    }
    const spinoffsId = new ObjectId(req.params.id);
...
const deleteSpinoffs = async (req, res) => {
    if (!ObjectId.isValid(req.params.id)) {
        res.status(400).json('Must use a valid spinoff id to delete a spinoff');
    }
    const spinoffId = new ObjectId(req.params.id);
